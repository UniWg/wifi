...
char buffer_trasmissione_vuoto(sta_registry_t* reg) { 	/* bisogna controllare se la Fifo è vuota*/
	...
	x = (*reg).ns;
	printf("%d", x);
	if (((*reg).BLT [0] == 0) && ((*reg).BLT [1] == 2) && ((*reg).BLT [2] == 1)) {	/* Sequenza che indica buffer vuoto */
...
char buffer_allocato(sta_registry_t* reg) {
	...
	if (fifo_is_empty((*reg).LTT [x+1])) {			/* Se la lista temporanea di trasmissione è vuota */
		return FALSE;
	}
	else return TRUE;
...
void prepara_buffer(sta_registry_t* reg){
	...
	if (fifo_read(LTL [x+1], fb)) {	/*  Dato che c'è qualcosa da leggere lo trasferisco nella lista temporanea di trasmissione*/

		fifo_push((*reg).LTT [x+1], fb);	
