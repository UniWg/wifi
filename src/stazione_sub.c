...
char frame_completo (int n, sta_registry_t* reg) {
	...
	if (n<_pframe_other_len)				/* Dobbiamo avere sufficienti caratteri per leggere la len del pacchetto */
		return (FALSE); 	
...
int sta_prendi_pacchetto (stato_sta_t *s, sta_registry_t* reg) {
	int n, ns;
	char pack[_max_frame_buffer_size];				/* struttura d'appoggio */
	...
	(*reg).x = appendi_pacchetto((*reg).BLR, pack, (*reg).x, n, ns);
...
void prepara_buffer(sta_registry_t* reg){
	...
	if (fifo_read(LTL[x], fb)) {	/*  Dato che c'è qualcosa da leggere lo trasferisco nella lista temporanea di trasmissione*/

		fifo_push((*reg).LTT, fb);	
...
void spedisci_CTS(sta_registry_t* reg, stato_sta_t *s) {
	...
	(*f).duration = 5;	
	(*f).packetl = _pframe_other_len;				/* Lunghezza base del pacchetto (non ci sono dati) */
	strncpy ((*f).addr1, (*g).addr2,6);				/* mac6 del destinatario */
	strncpy ((*f).addr2,stazione_g [ns].mac,6);		/* mac address della stazione che sta trasmettendo */
	strncpy ((*f).addr3,"000000",6);
	strncpy ((*f).addr4,"000000",6);
	(*f).seqctrl = 1;
...
void spedisci_pacchetto(sta_registry_t* reg, stato_sta_t *s) {
	...
	if (fifo_read((*reg).LTT, (*reg).BLT)) {		/* Se c'è un elemento nella lista lo metto nel buffer di trasmissione e spedisco */
		...
		while( (n = write(stafd_g [ns], &((*reg).BLT[nwrite]), len-nwrite)) >0 ) {
			printf( "BYTE %d \t", n);
			nwrite+=n;	
		}
