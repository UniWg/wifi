...
#define _ack_value 99999			/* valore che serve per riconoscere ack all'interno di seqctrl */

#define _packet_duration_low 500	/* Durata per CTS, RTS e pacchetti < 100 byte */
#define _packet_value_low 5			/* Valore */
#define _packet_duration_hi 2000	/* Durata per gli altri pacchetti */
#define _packet_value_hi 20			/* Valore */

#define _rts_packet_type 1			/* Pacchetto di tipo RTS */
#define _cts_packet_type 2			/* Pacchetto di tipo CTS */
#define _ack_packet_type 3			/* Pacchetto di tipo ACK */
...
* Par. Formali  :
			...
			- mitt_mac6 : numero del mittente (1..4)
			- dest_mac6 : numero del destinatario (1..4)
---------------------------------------------------------------------------- */
char* make_pseduo_frame (char* messaggio,int msglen,int seqctrl,char* mitt_mac6,char* dest_mac6,char prog_crc);

/* ----------------------------------------------------------------------------
* Nome			: carlo
* Descrizione	: Crea un pacchetto RTS, CTS o ACK a seconda del valore impostato
* Par. Ritorno  : Restituisce il puntatore all'area di memoria contenente il pacchetto
* Par. Formali  :
			- tipo : tipo di pacchetto da creare
			- mitt : mittente mac6
			- dest : destinatario mac6
---------------------------------------------------------------------------- */
char* pacchetto_di_controllo (char tipo,char* mitt,char* dest);

/* ----------------------------------------------------------------------------
* Nome			: carlo
* Descrizione	: Ricava dal pacchetto la dimensione indicata nel campo packetl
* Par. Ritorno  : Restituisce la dimensione del pacchetto
* Par. Formali  :
			- buf : pacchetto dati 
---------------------------------------------------------------------------- */
